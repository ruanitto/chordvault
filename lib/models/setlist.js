...
function applyDateFilters(bodyPart, bodyParams, dateFrom, dateTo) {
...
module.exports = {
  ...
  listForUser(userId, { q, dateFrom, dateTo, page = null, limit = null } = {}) {
    ...
    bodyPart = applyDateFilters(bodyPart, bodyParams, dateFrom, dateTo);

    return paginateQuery({
      selectPart, bodyPart, bodyParams, page, limit,
      countExpr: 'COUNT(DISTINCT s.id) as count',
      groupOrder: 'GROUP BY s.id ORDER BY s.event_date DESC, s.updated_at DESC',
      itemsKey: 'setlists',
    });
  },
