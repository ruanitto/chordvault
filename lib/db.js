...
db.prepare("UPDATE users SET gemini_model = 'gemini-3.1-flash-lite' WHERE gemini_model = 'gemini-3.1-flash-lite-preview'").run();

/**
 * Runs a paginated query or returns all rows.
 *
 * @param {object} opts
 * @param {string} opts.selectPart - SELECT columns clause
 * @param {string} opts.bodyPart - FROM/WHERE/JOIN clause
 * @param {string} opts.groupOrder - GROUP BY / ORDER BY suffix
 * @param {string} opts.countExpr - COUNT expression (default: 'COUNT(*) as count')
 * @param {any[]}  [opts.selectParams] - params bound before bodyParams in the data query
 * @param {any[]}  opts.bodyParams - params for the WHERE clause
 * @param {number|null} opts.page
 * @param {number|null} opts.limit
 * @param {string} opts.itemsKey - key name in the paginated result (default: 'items')
 * @param {number} [opts.defaultLimit] - limit when no pagination is requested (optional)
 * @returns {object|array}
 */
function paginateQuery({ selectPart, bodyPart, groupOrder, countExpr = 'COUNT(*) as count', selectParams = [], bodyParams, page, limit, itemsKey = 'items', defaultLimit }) {
  if (page !== null && limit !== null) {
    const totalCount = db.prepare(`SELECT ${countExpr} ${bodyPart}`).get(...bodyParams).count;
    const offset = (page - 1) * limit;
    const rows = db.prepare(`${selectPart} ${bodyPart} ${groupOrder} LIMIT ? OFFSET ?`).all(...selectParams, ...bodyParams, limit, offset);
    return {
      [itemsKey]: rows,
      total: totalCount,
      page,
  ...
  const suffix = defaultLimit ? `${groupOrder} LIMIT ${defaultLimit}` : groupOrder;
  return db.prepare(`${selectPart} ${bodyPart} ${suffix}`).all(...selectParams, ...bodyParams);
}
